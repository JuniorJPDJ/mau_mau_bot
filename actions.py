...
from utils import display_name, game_is_running
...
async def do_play_card(bot, player, result_id):
    ...
    if game.choosing_color:
        await bot.send_message(chat.id, text=__("Please choose a color", multi=game.translate))

    if len(player.cards) == 1:
        await bot.send_message(chat.id, text="UNO!")
...
async def do_draw(bot, player):
    ...
    try:
        ...
        await bot.send_message(player.game.chat.id,
