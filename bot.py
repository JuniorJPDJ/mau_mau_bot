...
from telegram import InlineKeyboardMarkup, \
    InlineKeyboardButton, Update, InlineQueryResultsButton
from telegram.ext import InlineQueryHandler, ChosenInlineResultHandler, \
    CommandHandler, MessageHandler, CallbackQueryHandler, CallbackContext, filters
...
from utils import error, user_is_creator_or_admin, user_is_creator, game_is_running
...
async def kill_game(update: Update, context: CallbackContext):
    ...
    if user_is_creator_or_admin(user, game, context.bot, chat):
        ...
        try:
            ...
            await context.bot.send_message(chat.id, text=__("Game ended!", multi=game.translate))
...
async def join_game(update: Update, context: CallbackContext):
    ...
    try:
        await gm.join_game(update.message.from_user, chat)

    except LobbyClosedError:
            await context.bot.send_message(chat.id, text=_("The lobby is closed"))
...
async def leave_game(update: Update, context: CallbackContext):
    ...
    if player is None:
        await context.bot.send_message(chat.id, text=_("You are not playing in a game in "
                                        "this group."),
                   reply_to_message_id=update.message.message_id)
        return
...
async def kick_player(update: Update, context: CallbackContext):
    ...
    if user_is_creator_or_admin(user, game, context.bot, chat):
        ...
        if update.message.reply_to_message:
            ...
            try:
                await gm.leave_game(kicked, chat)

            except NoGameInChatError:
                await context.bot.send_message(chat.id, text=_("Player {name} is not found in the current game.".format(name=display_name(kicked))),
                                reply_to_message_id=update.message.message_id)
                return
...
async def status_update(update: Update, context: CallbackContext):
    ...
    if update.message.left_chat_member:
        ...
        try:
            ...
            await gm.end_game(chat, user)
            await context.bot.send_message(chat.id, text=__("Game ended!",
                                             multi=game.translate))
        else:
            await context.bot.send_message(chat.id, text=__("Removing {name} from the game",
                                             multi=game.translate)
...
async def start_game(update: Update, context: CallbackContext):
    ...
    if update.message.chat.type != 'private':
        ...
        if game.started:
            await context.bot.send_message(chat.id, text=_("The game has already started"))
...
async def close_game(update: Update, context: CallbackContext):
    ...
    if user.id in game.owner:
        game.open = False
        await context.bot.send_message(chat.id, text=_("Closed the lobby. "
...
async def open_game(update: Update, context: CallbackContext):
    ...
    if user.id in game.owner:
        game.open = True
        await context.bot.send_message(chat.id, text=_("Opened the lobby. "
...
async def enable_translations(update: Update, context: CallbackContext):
    ...
    if user.id in game.owner:
        game.translate = True
        await context.bot.send_message(chat.id, text=_("Enabled multi-translations. "
...
async def disable_translations(update: Update, context: CallbackContext):
    ...
    if user.id in game.owner:
        ...
        await context.bot.send_message(chat.id, text=_("Disabled multi-translations. "
...
async def reply_to_query(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if not game.started:
            if user_is_creator(user, game):
                add_mode_classic(results, player.anti_cheat)
            ...
            if game.choosing_color:
                add_choose_color(results, game, player.anti_cheat)
                add_other_cards(player, results, game, player.anti_cheat)
            else:
                if not player.drew:
                    add_draw(player, results, player.anti_cheat)

                else:
                    add_pass(results, game, player.anti_cheat)

                if game.last_card.special == c.DRAW_FOUR and game.draw_counter:
                    add_call_bluff(results, game, player.anti_cheat)
                ...
                add_gameinfo(game, results, player.anti_cheat)

        elif user_id != game.current_player.user.id or not game.started:
            for card in sorted(player.cards):
                add_card(game, card, results, can_play=False, anti_cheat=player.anti_cheat)
    ...
    kwargs = {"cache_time": 0}
    if switch:
        kwargs["button"] = InlineQueryResultsButton(text=switch, start_parameter='select')
    await context.bot.answer_inline_query(update.inline_query.id, results, **kwargs)
...
async def process_result(update: Update, context: CallbackContext):
    ...
    if result_id in ('hand', 'gameinfo', 'nogame'):
        ...
        # The first 5 characters are 'mode_', the rest is the gamemode.
        mode = result_id[5:]
        game.set_mode(mode)
        logger.info("Gamemode changed to {mode}".format(mode = mode))
        await context.bot.send_message(chat.id, text=__("Gamemode changed to {mode}".format(mode = mode)))
        return
