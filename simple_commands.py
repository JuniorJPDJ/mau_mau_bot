...
async def modes(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(update.message.chat_id, text=modes_explanation,
...
async def source(update: Update, context: CallbackContext):
    ...
    attributions = _("Attributions:\n"
      'Draw icon by '
      '<a href="https://www.faithtoken.com/">Faithtoken</a>\n'
      'Pass icon by '
      '<a href="https://delapouite.com/">Delapouite</a>\n'
      "Originals available on https://game-icons.net\n"
      "Icons edited by ɳick")

    await context.bot.send_message(update.message.chat_id, text=source_text + '\n' +
                                                 attributions,
               parse_mode=ParseMode.HTML, disable_web_page_preview=True)
